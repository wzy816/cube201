...
static const unsigned char edge_twist_perm[FACES][4] = {
    {0, 2, 3, 1}, {3, 7, 11, 6}, {2, 5, 10, 7}, {9, 11, 10, 8}, {0, 4, 8, 5}, {1, 6, 9, 4}};

// affected slot index by corner permutataion
static const unsigned char corner_twist_perm[FACES][4] = {
    {0, 1, 3, 2}, {2, 3, 7, 6}, {3, 1, 5, 7}, {4, 6, 7, 5}, {1, 0, 4, 5}, {0, 2, 6, 4}};
...
void cubepos::init()
{
  ...
  // init inv_move array = 2,1,0,5,4,3,8,7,6,11,10,9,14,13,12,17,16,15
...
// invert a move seq by invert every move
vector<int> cubepos::invert_sequence(const vector<int> &seq)
{
  unsigned int len = seq.size();
  vector<int> r(len);
  for (unsigned int i = 0; i < len; i++)
    r[len - i - 1] = inv_move[seq[i]];
  return r;
}
...
//
void cubepos::append_moveseq(char *&p, const vector<int> &seq)
{
  *p = 0;
...
//
vector<int> cubepos::parse_moveseq(const char *&p)
{
  vector<int> r;
  int mv;
...
//
char *cubepos::moveseq_string(const vector<int> &seq)
{
  char *p = static_buf;
